 ...
 */
abstract class DeprecatedRecyclerSectionAdapter<T>(protected var context: Context) : RecyclerView.Adapter<BaseViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder {
        if (viewType == SECTION_TYPE) {
            ...
            if (customView != null && titleForSection(0) == null) {
                return BaseViewHolder(customView)
            } else {
                val view = LayoutInflater.from(context).inflate(R.layout.section_view, parent, false)
                return SectionBaseViewHolder(view, R.id.section_text)
            }
        } else {
    ...
    override fun onBindViewHolder(holder: BaseViewHolder, position: Int) {
        if (isSectionAt(position)) {
            ...
            if (holder is SectionBaseViewHolder) {
...
/*        try {
            ...
            for (item in items) {
                try {
                    val currentTitle = sortProperty.toString()
                    if (currentTitle != previousTitle) {
                        sections.add(Section(items.indexOf(item), currentTitle))
                        previousTitle = currentTitle
                    }
    ...
    class SectionBaseViewHolder(view: View, textResourceid: Int) : BaseViewHolder(view) {
